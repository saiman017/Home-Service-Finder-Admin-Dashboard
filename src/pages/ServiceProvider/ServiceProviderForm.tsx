...
// import { fetchServiceProviderById, updateServiceProvider } from "../../store/slice/serviceProvider";
...
//       await dispatch(updateServiceProvider({ id: providerId, updateData })).unwrap();
//       notifySuccess("Provider updated successfully");
//       onClose();
//       afterSubmit?.();
//     } catch (err) {
//       console.error("Update error", err);
//       notifyError("Failed to update provider");
//     }
//   };
...
//               <Field name="serviceCategoryId" as={Input} />
...
const ServiceProviderFormik: React.FC<Props> = ({ providerId, onClose, afterSubmit, initialData }) => {
  ...
  const handleSubmit = async (values: typeof initialValues, { resetForm }: FormikHelpers<typeof initialValues>) => {
    ...
    try {
      // rebuild your payload exactly as before
      const updateData = {
        ...values,
        dateOfBirth: dayjs(values.dateOfBirth).format("YYYY-MM-DD"),
      };

      // 1️⃣ dispatch & unwrap full response
      ...
      // 4️⃣ both server-validation and network/HTTP errors land here
      notifyError(err.message || "Failed to update provider");
    } finally {
      // 5️⃣ always reset, reload list, and close modal
      resetForm();
      dispatch(fetchAllServiceProviders());
      onClose();

      // 6️⃣ only fire afterSubmit if it truly succeeded
      if (didSucceed) {
        afterSubmit?.();
      }
    }
