export const decodeBase64Data = (base64EncodedData: any) => {
  if (typeof base64EncodedData === "string" && base64EncodedData.length > 0) {
    try {
      ...
      if (parsedData.AccessToken && parsedData.RefreshToken && parsedData.IsEmailVerified) {
        // const { AccessToken, RefreshToken, Username, Role, Email } = parsedData;

        const { AccessToken, RefreshToken, Role, Email, IsEmailVerified, UserId } = parsedData;
